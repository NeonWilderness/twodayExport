          ...
          complete: function () {
            var xhr = $.get('/topics', function (data) {
              ...
              $('#btnVersion').on('click', function () {
                var yql = 'https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20json%20where%20url%3D%22http%3A%2F%2Fstatic.twoday.net%2FNeonWilderness%2Ffiles%2Fversion.json%22&format=json&callback=';
                $.getJSON(yql, function (data) {
                  var newVersion = false, release, msgClass, msgText;
                  if (data.query.count > 0) {
                    $.each(data.query.results.json.packages, function () {
